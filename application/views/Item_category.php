			...
			<div class='row wrapper border-bottom white-bg page-heading' id=''>
				<div class='col-lg-10'>
					...
					<ol class='breadcrumb'>
				    	...
				    	<li class='breadcrumb-item'><a href=''>Add and View Item_category</a></li>
			    	</ol>
			   </div> 
			</div>
                ...
                <div class='ibox '>
	                <div class='ibox-title'>
						<div title='Click to add new category'>
							<a href='javascript:void(0)' onclick='add_Item_category()' class='btn btn-w-m btn-primary' data-toggle='modal'> Add</a>
						</div>
					</div>
                    <div class='ibox-title'>
                        <h5>List of Item_category</h5>
        ...
        {
        	$('#myModalItem_category').modal();
		$('#mymodal_body').html('Loading...');
		 $.ajax({url:"<?php echo site_url('Ajaxs/Masters_ajax/new_Item_category');?>" ,type: 'GET',async:false, success:function(result){
		 	$('#mymodal_Item_categorybody').html(result);
		 	}
		 });
        }
        function edit_Item_category(id)
        {
        	param = '?id='+id;
        	$('#myModalItem_category').modal();
			$('#Item_category_body').html('Loading...');
        	$.ajax({url:"<?php echo site_url('Ajaxs/Masters_ajax/edit_Item_category');?>"+param ,type: 'GET',async:false, success:function(result){
		 	$('#mymodal_Item_categorybody').html(result);
		 	}
		 });
        }
        function delete_Item_category(id)
        {
            ...
            { 
	        	param = '?id='+id;
	        	$.ajax({url:"<?php echo site_url('Ajaxs/Masters_ajax/delete_Item_category');?>"+param ,type: 'GET',async:false, success:function(result){
			 			$('#Item_category_body').html(result);
			 	}
			 	});
		 	}
        }
        </script>
        <div class='modal inmodal' id='myModalItem_category' tabindex='-1' role='dialog'  aria-hidden='true'>
		 	<div class='modal-dialog'>
		 		<div class='modal-content animated fadeIn'>
					<div class='modal-header'>
						<button type='button' class='close' data-dismiss='modal'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button>
							<h4 class='modal-title'>Add and Edit Item_category</h4>
					</div>
					<div class='modal-body' id='mymodal_Item_categorybody'>
